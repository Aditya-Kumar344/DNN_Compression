...
# ── Fix Problem 2: Vectorized LBP (no Python loops) ─────────────────────────
def get_lbp(gray):
    """Vectorized LBP using NumPy shifts — ~100x faster than pixel loop"""
    h, w   = gray.shape
    lbp    = np.zeros((h, w), dtype=np.uint8)
    center = gray[1:h-1, 1:w-1]

    # 8 neighbours using array slicing instead of per-pixel loop
...
class Fruit360_DataLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # ── Fix Problem 1: Safe image loading ───────────────────────────
        try:
            ...
            # Return a blank sample instead of crashing
            return {
        ...
        # Fix Problem 2: vectorized LBP called here
        lbp   = get_lbp(gray)
        ...
        # ── Fix Problem 4: RGB never returned — not used by model ────────
        ...
        return {
            # "rgb" intentionally removed — model doesn't use it
            "lbp"           : lbp,            # (1, 100, 100)
            "canny"         : canny,           # (1, 100, 100)
            "color_features": color_features,  # (6,)
